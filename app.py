...
# Malla estratégica exclusiva para 50A y 50B (Línea 50R eliminada)
PARADAS_RADAR = [
    # Cabeceras
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    ...
    /* Transición mágica de Leaflet para que los coches se deslicen fluidamente en 1 segundo */
...
<body>
  ...
  <script>
    ...
    const VELOCIDAD_M_S = 5.5; // ~20 km/h urbanos
    ...
    function initMap() {
      ...
      // Activa el motor predictivo de movimiento fluido (1 vez por segundo)
      setInterval(tickPredictivo, 1000);
    }

    // Motor Predictivo (Dead Reckoning): Adelanta el coche sobre la ruta cuando Indalo no manda datos
    ...
    function actualizarMapa(buses) {
      ...
      buses.forEach(b => {
        ...
        if (marcadores[b.id]) {
          ...
          b.targetIdx = null; // Resetea el predictor para que parta del nuevo nodo real
        } else {
...
def api_arribos():
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(consultar_arribos_worker, p) for p in consultas_a_ejecutar]
    ...
    # 1. Deduplicación intra-ronda: 45 metros de tolerancia (Permite ver caravanas)
    buses_unicos_ronda = []
    ...
    # 2. Seguimiento de ruta (900m de radio direccional)
